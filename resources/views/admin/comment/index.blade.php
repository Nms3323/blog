        ...
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Comment') }}
				...
				<ul class="breadcrumb breadcrumb-separatorless fw-bold fs-7 my-1">
					...
					<!--end::Item-->
					<!--begin::Item-->
					<li class="breadcrumb-item text-muted">Comment Details</li>
					<!--end::Item-->
				</ul>
				<!--end::Breadcrumb-->
			</dv>
	...
	<div class="toolbar" id="kt_toolbar">
		...
		<div id="kt_toolbar_container" class="container-fluid d-flex flex-end ml-auto mr-auto">
			...
			<div class="d-flex align-items-center py-1">
				...
				<!--end::Wrapper-->
				<!--begin::Button-->
				<a href="{{route('admin.comment.create')}}" class="btn btn-sm btn-primary" id="kt_toolbar_primary_button">Create</a>
				<!--end::Button-->
			</div>
			<!--end::Actions-->
		</div>
			...
			<div class="d-flex flex-column flex-xl-row">
				...
				<div class="flex-lg-row-fluid ms-lg-0">
					<div class="table-responsive">
						<!--begin::Table-->
						<table class="table align-middle table-row-dashed fs-6 gy-5 data-table dataTableNew" id="kt_subscriptions_table">
							<!--begin::Table head-->
							<thead>
								<!--begin::Table row-->
							...
							<tbody class="text-gray-600 fw-bold">
								...
								<tr>
									<td>{{ $count++ }}</td>
									<td>{{ ucfirst($dt->blog_name) }}</td>
									<td>{{ ucfirst($dt->user_name) }}</td>
									...
									<td>
										<a class="btn btn-sm btn-clean btn-icon btn-icon-md" title="Edit" href="{{route('admin.comment.edit', $dt->id)}}" style="background: #968cbd;">
											<i class="la la-edit" style="color: white !important;"></i>
										</a>
										<input type="hidden" value="{{ $dt->id }}" name="delete_comment_id" id="delete_comment_id">
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		{
			var obj = $(this);
			var id = $(this).closest('td').find("#delete_comment_id").val();
			var delete_url = "{{route('admin.comment.destroy', ':id') }}";
