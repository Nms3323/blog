        ...
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Update Comment') }}
				...
				<ul class="breadcrumb breadcrumb-separatorless fw-bold fs-7 my-1">
					...
					<!--end::Item-->
					<!--begin::Item-->
					<li class="breadcrumb-item text-muted">Update Comment</li>
					<!--end::Item-->
				</ul>
				<!--end::Breadcrumb-->
			</dv>
			...
			<div class="d-flex flex-column flex-xl-row">
				...
				<div class="row">
					<div class="col-md-12 pe-lg-10">
						<!--begin::Form-->
						<form action="{{ route('admin.comment.update', $comment_data->id) }}" method="post" class="form mb-15 add__form" id="kt_contact_form">
							<!--begin::Input group-->
							{{ method_field('PATCH') }}
							@csrf
							<input type="hidden" name="hidden_id" value="{{$comment_data->id}}">
							...
							<div class="row mb-5">
								...
								<div class="col-md-5 fv-row">
									...
									<select class="form-control" name="user_id" id="user_id" required>
										...
										@if($user_list_dt->id == $comment_data->user_id)
										@php $selected_user = "selected"; @endphp
										@endif
										<option {{$selected_user}} value="{{$user_list_dt->id}}">{{$user_list_dt->name}}--({{$user_list_dt->email}})</option>
										@endforeach
										@endif
									</select>
									<!--end::Input-->
								...
								<div class="col-md-7 fv-row">
									...
									<select class="form-control" name="blog_id" id="blog_id" required>
										...
										@php $selected_blog = ""; @endphp
										@if($user_list_dt->id == $comment_data->blog_id)
										@php $selected_blog = "selected"; @endphp
										@endif
										<option {{$selected_blog}} value="{{$blog_list_dt->id}}">{{$blog_list_dt->name}}</option>
										@endforeach
										@endif
									</select>
									<!--end::Input-->
